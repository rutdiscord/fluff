    ...
    async def get_all_sticky_messages(self) -> dict[int, dict[int, StickyEntry]]:
        ...
        async with self.db.get_read_connection() as conn:
            cursor = await conn.execute(
                "SELECT server_id, channel_id, message, last_message_id, repost_frequency FROM sticky_message"
            )
            rows = await cursor.fetchall()

        for row in rows:
            server_id, channel_id, message, last_message_id, repost_frequency = row
            ...
            sticky_messages[server_id][channel_id] = StickyEntry(message, last_message_id, repost_frequency)

        return sticky_messages

    async def create_sticky_message(self, server_id: int, channel_id: int, repost_frequency: int, message: str) -> tuple[int, int]:
        ...
        async with self.db.get_write_connection() as conn:
            await conn.execute(
                "INSERT INTO sticky_message (server_id, channel_id, repost_frequency, message, last_message_id) "
                "VALUES (?,?,?,?,?)",
                (str(server_id), str(channel_id), repost_frequency, message, None)
            )
            await conn.commit()

            return server_id, channel_id

    async def update_sticky_message(self, server_id: int, channel_id: int, repost_frequency: int, message: str | None) -> None:
        """Updates an existing sticky message's repost_frequency column, and, optionally, the message column."""
        async with self.db.get_write_connection() as conn:
            if message is not None:
                await conn.execute(
                    "UPDATE sticky_message SET repost_frequency = ?, message = ? WHERE server_id = ? AND channel_id = ?",
                    (repost_frequency, message, str(server_id), str(channel_id))
                )
            else:
                await conn.execute(
                    "UPDATE sticky_message SET repost_frequency = ? WHERE server_id = ? AND channel_id = ?",
                    (repost_frequency, str(server_id), str(channel_id))
                )
            await conn.commit()
